...
def tire_gauge_realtime(serial):
    ...
    json_response = do_request("v5/tire_gauge/realtime", add_params={"sn": serial}).get(
        "data"
    )
...
def tire_gauge_status(serial):
    ...
    json_response = do_request("v5/tire_gauge/status", add_params={"sn": serial}).get(
        "data"
    )
...
def track_list_v2(serial, pagesize=10, index=0):
    ...
    json_response = do_request(
        "v5/k/track_list",
        method="get",
        add_params={"index": str(index), "pageSize": pagesize, "sn": serial},
        add_headers={
            "user-agent": "manager/4.6.44 (nuiAPI);lang=en-US;clientIdentifier=Overseas"
        },
    ).get("data")
